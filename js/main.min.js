...
//                   console.log(file);
//                    console.log($("#chooseColumn").closest('div'));
                    console.log(store);
                    $(".classChooseColumn").empty();
                    for (var b in logs.headers){ 
//                        console.log(logs.headers[b]);
                        $("#chooseColumn").append("<li><input type=checkbox onclick=myFunc2('"+file+"','"+logs.headers[b]+"') value='"+logs.headers[b]+ "'/>"+logs.headers[b]+"</li> ");
                        if (file in store){
                            if (logs.headers[b] in store[file]){
                                if (store[file][logs.headers[b]]==1)
                                    $("#chooseColumn input[value="+logs.headers[b]+"]").prop('checked', true);
                            }else{
                                store[file][logs.headers[b]]=1;
                                $("#chooseColumn input[value="+logs.headers[b]+"]").prop('checked', true);
                            }
                        }else{
                            store[file]={};
                        ...
                        $("<th>" + logs.headers[b] + "</th>").addClass(b).appendTo(a);
                    }
                    console.log(file);
            ...
            for (var log in logs.logs) {
                ...
                for (var c in logs.logs[log])
                    if ("pml" !== c) {
                        ...
                        } else if ("date" === type.parser) title = logs.logs[log].pml, val = val_cut(val, type.cut);
                        ...
                        } else val = "prefake" === type.parser ? val_cut(val.replace(/(?:\r\n|\r|\n)/g, "<br/>"), type.cut) : val_cut(val, type.cut);
        ...
        /*
         * myFunc3 is called after the everything is loaded
         */
        myFunc3(file);
...
$(function () {
    ...
    }), notify(), $("#logsbody").click(function (a) {
        var b = $(a.target);
        $(b).hasClass("pml-date") && ($(b).siblings().toggleClass("marker"), $(b).toggleClass("marker"))
    }), $("#clear-markers").click(function () {
        $(".marker").removeClass("marker")
    }), pull_to_refresh === !0 && $("#hook").hook({
        dynamic: !1,
        reloadPage: !1,
        reloadEl: function () {
            notify(), get_logs()
...
function myFunc3(file){
//    console.log("here");
    var count=0;
    $(".multiSel").empty();
//    console.log("now")
//    console.log(store);
    for (title in store[file]){
        if (store[file][title]==1){
            count++;
            var html = '<span title="' + title + '">' + title +","+ '</span>';
            $('.multiSel').append(html);
            $("."+file+" th[class="+title+"]").show();
            $(".pml-"+title).show();
            
        }else{
            $("."+file+" th[class="+title+"]").hide();
            $(".pml-"+title).hide();
        }
    }if (count==0){
        $(".hida").show();
...
function myFunc2(file, value){
    var ele = $("#chooseColumn input[value="+value+"]");
    if ($(ele).is(':checked')) {
        store[file][value]=1;
    } 
    else {
...
////    var encoded = bto a(json);
//    var xhr = new XMLHttpRequest();
//    xhr.open('POST','chooseColumn.php',true);
